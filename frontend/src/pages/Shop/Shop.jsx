...
const Shop = () => {
  const { url, limited_shop_list, limited_flowers_list, fetchFlowersListWithPagination, fetchFlowersList, flowerPagination, listRatings, fetchRatingsList, totalRatings, } = useContext(StoreContext);
  ...
  return (
    <div className='shop-container'>
      <div className="shop-info">
        ...
        <p className="shop-rating"><img src={assets.daimond} alt="Рейтинг" className="star-logo-img" />{shop.averageRating}</p>
      </div>
      ...
      <div className="shop-display">
        <div className="shop-display-top">
          ...
          <div className='shop-center-information'>
            <div className='shop-work__information'>
              <div className='shop-icon__time'>
                <img className="time-icon" src={assets.time} alt="" /> <p className='work-time-desc'> Режим работы:</p>
              </div>
              <div>
            ...
            <div className='shop-payment__information'>
              <div className='shop-icon__time'>
                <img className="time-icon" src={assets.payment_icon} alt="" /> <p className='work-time-desc'> Форма оплаты:</p>
              </div>
          ...
          <div className='flower-display-shop'>
            ...
            <p className="shop-information">Адрес: {shop.address}</p>
            
            <p className='shop-information'>Доставка курьерской службой: {shop.delivery === true ? "Есть" : "Нет"}</p>
          </div>
        </div>
        <hr className='shop-info-divider' />
        <div className="flower-display-list">
          {limited_flowers_list.length > 0 ? (
            limited_flowers_list.map((flower) => (
              <FlowerItem key={flower._id} {...flower} />
            ))
          ) : (
      ...
      <div className='rating-window'>
        ...
        <Link to={`/shop/${shopId}/rating/`}><button className="shop-item-button rating-button-shop ">Все отзывы</button></Link>
      </div>
    </div>
  );
};
